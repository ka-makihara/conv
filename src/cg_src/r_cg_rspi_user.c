...
extern const uint32_t * gp_rspi2_tx_address;         /* RSPI2 transmit buffer address */
extern uint16_t         g_rspi2_tx_count;            /* RSPI2 transmit data number */
extern uint32_t *       gp_rspi2_rx_address;         /* RSPI2 receive buffer address */
extern uint16_t         g_rspi2_rx_count;            /* RSPI2 receive data number */
extern uint16_t         g_rspi2_rx_length;           /* RSPI2 receive data length */
...
void r_rspi1_receive_interrupt(void)
{
    ...
    for (frame_cnt = 0U; frame_cnt < (_RSPI_FRAMES_1 + 1U); frame_cnt++)
    {
        ...
        {
            *(uint32_t *)gp_rspi1_rx_address = RSPI1.SPDR.LONG;
            gp_rspi1_rx_address++;
            g_rspi1_rx_count++;
...
/***********************************************************************************************************************
* Function Name: r_rspi2_transmit_interrupt
* Description  : This function is SPTI2 interrupt service routine.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void r_rspi2_transmit_interrupt(void)
{
    uint16_t frame_cnt;
        
    /* Clear the interrupt source */
    VIC.PIC2.LONG = 0x02000000UL;
    ...
    for (frame_cnt = 0U; frame_cnt < (_RSPI_FRAMES_1 + 1U); frame_cnt++)
    {
        ...
        {
            /* Write data for transmission */
            RSPI2.SPDR.LONG = (*(uint32_t*)gp_rspi2_tx_address);
            gp_rspi2_tx_address++;
            g_rspi2_tx_count--;
        }
        else
...
/***********************************************************************************************************************
* Function Name: r_rspi2_receive_interrupt
* Description  : This function is SPRI2 interrupt service routine.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void r_rspi2_receive_interrupt(void)
{
    uint16_t frame_cnt;
    
    /* Clear the interrupt source */
    VIC.PIC2.LONG = 0x01000000UL;

    for (frame_cnt = 0U; frame_cnt < (_RSPI_FRAMES_1 + 1U); frame_cnt++)
    {
        if (g_rspi2_rx_length > g_rspi2_rx_count)
        {
            *(uint32_t *)gp_rspi2_rx_address = RSPI2.SPDR.LONG;
            gp_rspi2_rx_address++;
            g_rspi2_rx_count++;
            ...
            {
                /* Disable receive interrupt */
                RSPI2.SPCR.BIT.SPRIE = 0U;
                r_rspi2_callback_receiveend();
                break;
            }
        }
    }
...
/***********************************************************************************************************************
* Function Name: r_rspi2_error_interrupt
* Description  : This function is SPEI2 interrupt service routine.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void r_rspi2_error_interrupt(void)
{
    uint8_t err_type;
    ...
    /* Disable error interrupt */
    RSPI2.SPCR.BIT.SPEIE = 0U;

    /* Disable idle interrupt */
    RSPI2.SPCR2.BIT.SPIIE = 0U;

    /* Clear error sources */
    err_type = RSPI2.SPSR.BYTE;
    RSPI2.SPSR.BYTE = 0xA0U;

    if (err_type != 0U)
    {
        r_rspi2_callback_error(err_type);
    }
    /* Wait the interrupt signal is disabled */
    while (0U != (VIC.IRQS2.LONG & 0x04000000UL))
    {
        VIC.IEC2.LONG = 0x04000000UL;
    }
    
    VIC.IEN2.LONG |= 0x04000000UL;
...
/***********************************************************************************************************************
* Function Name: r_rspi2_idle_interrupt
* Description  : This function is SPII2 interrupt service routine.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
void r_rspi2_idle_interrupt(void)
{
    /* Disable RSPI function */
    RSPI2.SPCR.BIT.SPE = 0U;
    ...
    r_rspi2_callback_transmitend();
    
    /* Wait the interrupt signal is disabled */
    while (0U != (VIC.IRQS2.LONG & 0x08000000UL))
    {
        VIC.IEC2.LONG = 0x08000000UL;
    }
    
    VIC.IEN2.LONG |= 0x08000000UL;
...
/***********************************************************************************************************************
* Function Name: r_rspi2_callback_transmitend
...
/***********************************************************************************************************************
* Function Name: r_rspi2_callback_receiveend
* Description  : This function is a callback function when RSPI2 finishes reception.
* Arguments    : None
* Return Value : None
***********************************************************************************************************************/
...
/***********************************************************************************************************************
* Function Name: r_rspi2_callback_error
* Description  : This function is a callback function when RSPI2 error occurs.
* Arguments    : err_type -
*                    error type value
* Return Value : None
***********************************************************************************************************************/
