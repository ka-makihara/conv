...
int endat_pos(uint32_t *posUpper, uint32_t *posLower);
...
int endat_pos(uint32_t *posUpper, uint32_t *posLower)
{
	...
	if (ENDAT_SUCCESS == err_code){
		...
		if (ENDAT_REQ_SUCCESS == pendat_result->result){
			pos_upper = pendat_result->data.pos >> ENDAT_SHIFT_32;
			pos_lower = pendat_result->data.pos & ENDAT_MASK_LOW_32;

			if( posUpper != 0 && posLower != 0){
				*posUpper = pos_upper;
				*posLower = pos_lower;
			}
